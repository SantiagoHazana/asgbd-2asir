...
-- c)
create view languagesOfCountires as select c.name as Country, cl.official as Official, l.language from countries c
    inner join country_languages cl on c.country_id = cl.country_id
    inner join languages l on cl.language_id = l.language_id;

-- d)
create view population_density as select name, area, population, (population/area) as densidad from countries
